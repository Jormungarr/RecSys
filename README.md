...
待办（一次只做一件，完整顺序见 `memory.md` 的快照）：① 阶段 3 剩下的两臂（模型层，一次一臂：C `is_organic` 当事件特征、D 艺人-专辑 embedding；**都在 d64/2 原始架构上做**，才能与现有 baseline 直接比）→ ② 位置通道收尾（重复 mlp / t2v、上 d128/4 验证）→ ③ 拆开 EMB 与层数归因 → ④ dropout / 早停 → ⑤ 口径 B 的容量包。另：排名融合重做（旧结论已作废）、多 seed 定量噪声、BPR 基线、likes / dislikes 交互、服务化。
