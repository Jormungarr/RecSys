...
- **D（艺人 / 专辑 embedding）先要想清两件事**（2026-09-22 讨论）：
  ① **共线性**——`album ⊂ artist ⊂ item` 三层嵌套，item 表（40.2M 参数）本身就在学这种结构；加表的代价是 artist **5.3M**（+13%）、album **21.9M**（+55%）；
  真要试先只加**艺人**，且判读要看**长尾 / 新歌桶**而不是总平均（“给稀疏物品提供跨物品共享”才是它的真正赌注）。
  ② **外推**——离线口径把冷物品记 `-1`（val 1.34% / test 1.48%），而 artist/album embedding 对**真新艺人（253 / 298 个物品）完全无效**：
  表里根本没有那个 id。真冷启动只有**音频嵌入**（`embeddings.parquet`，阶段 4）能碰；若要量，得单开一条候选池含全部 Listen+ 物品的评测线（与现有历史表不可比）。
  → 两者的事实与出处见 `docs/dataset_notes.md`「未进管线的字段与文件」；**D 是否做、或改走阶段 4，等用户定**。
