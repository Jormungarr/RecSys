...
29. **阶段 3·第一臂：难负样本（`NEG_MODE`，不加参数不改架构）——负结果**（2026-09-22）：每个位置仍只抽 1 个负样本，但 `HARD_SHARE=0.5` 的位置换难：
  ① 该用户**观测到的负反馈**（`weak_negative` ∪ 训练窗口内 dislike；7,042,955 个物品 / 覆盖 9,198 用户 / 每用户中位 477 个）；
  ② **高流行未交互**（频次 top-2000 减完整历史）。实测难负样本平均频次 **2,214** vs 均匀 **45**；池只花 2.5 s 建一次；
  撞正样本退回（不守则有 0.11% 位置会撞）。同一条命令背靠背：基线 902.9 s / hard 962.8 s。
  **结果：`@100` 全线变差、`@10` 变好**——val recall@100 **0.091359 → 0.078416（−14.2%）**、命中率@100 −15.3%；
  test **0.085805 → 0.071054（−17.2%）**、命中率@100 −18.9%；回访 −20.6% / −22.3%；
...
31. **阶段 3·第三臂：is_organic 当事件特征（`USE_ORGANIC`）——小幅负结果**（2026-09-22）：`Embedding(2, EMB)` 加到 token 表示上，
  **+128 个参数**（本仓库第一个加参数的开关；参数总量 139,328 → 139,456），推理用输入序列的事件标记（不泄未来）；
  **新增第 8 处守卫**（与 checkpoint 里有无 `organic_embedding` 必须一致，忘了带开关跑 test 会被可读提示拦住）。
  两臂背靠背（基线 831.3 s / on 852.0 s，基线 loss 轨迹与前三臂逐位相同）。
  **结果**：主指标小幅变差——val recall@100 **0.091487 → 0.090572（−1.0%）**、命中率@100 −1.7%；test **0.085380 → 0.084087（−1.5%）**；
  回访 −2.2% / −2.4%；**唯一稳定收益是 新歌@10（+17.4% / +14.2%）**，val 上新歌@100 +12.7%、新歌·主动发现@100 +22.6%，但 test 上新歌@100 −3.7%。
  128 个参数不是容量问题 → 差异可归因到信号本身。**元观察：A / B / C 三臂都只把「新歌@10」推好（+32% / +8.6% / +14.2%），
  没有一个能稳定推动主指标（主指标 66.6% 是回访）**。→ `docs/baselines.md`「is_organic 当事件特征」
32. **长尾/稀疏度分层 + D 可行性检查（只读）——结论：D 不做**（2026-09-22）：`rec_eval` 加第四条轴 `回访×稀疏度`（目标物品训练集交互数 ≤5 / 6~100 / >100）。
  四条结论：① **新歌目标大多数不是长尾**（59~65% 是 >100 次的热门，只有 10~16% 稀疏）；
  ② **“新歌·稀疏(≤5)” 是一块三模型全为 0 的死区**（test 9,158 行 = 5.8%，含 2,332 行冷物品），新歌轴的成绩几乎全来自热门桶
  （sasrec 0.0503 / popularity 0.0581）；③ 同艺人物品对的共现余弦是随机的 **163×**（结构很强，所以 item 表已经在学它）；
  ④ 死区里有下手空间（用户历史已含同艺人）的只有 ≈**35%** → 上限约总目标 2%、总指标上看不见。
  （③ 以 `scripts/07_zone_checks.py` 去重后的口径为准：艺人 **0.313 / 163×**、专辑 **0.449 / 238×**。）
  **因此不做 D**（参数 530 万 / 2,190 万 + 三层嵌套共线）；要动死区只有音频嵌入（阶段 4）对症，但得先定口径。
  **脚本已入库**：用户 2026-09-22 要求“检查脚本和结果也是模型工作的一部分”，所以这四项检查固化成 `scripts/07_zone_checks.py`
  （只读、不训练；输出与本节数字一致，③ 去重后的口径以脚本为准：艺人 0.313/163×、专辑 0.449/238×）。
  这覆盖了之前“一次性的验证不固化成脚本”的做法（那条约定针对的是临时验证，本项已被明确要求入库）。
  → `docs/baselines.md`「长尾/稀疏度分层 + D 可行性检查」
33. **说明文档整理（用户 2026-09-22 要求“把改进模型的思考/实验/检查写明白 + 附指标对比”）**：README 新增两块——
  **「模型是怎么一步步改进的」**（15 行表：想法 → 怎么做 → 同 session 结果 → 结论，+ 三条贯穿全程的规矩 + 四个阶段叙述）与
  **「关键指标对比」**（各配置 val / test 汇总 + 分层轴关键数字）；旧的「当前状态」改写成「当前状态与下一步」（指针指向 `memory.md`）。
...
- **把没进管线的字段/文件囊括进来（用户 2026-09-22 定：这次都囊括）** —— 盘点与实测统计见 `docs/dataset_notes.md`「未进管线的字段与文件」，分四阶段：
  ...
  - **阶段 3（模型层，一次一臂）—— 进行中**：~~A 难负样本~~（负结果）、~~B 正样本加权~~（弱正、val 不可判定）、~~C is_organic 事件特征~~（小幅负结果）（见「已完成」29/30/31）；
...
- **D（艺人 / 专辑 embedding）不做**（2026-09-22 检查后定，见「已完成」32）：
  ① **共线性**——`album ⊂ artist ⊂ item` 三层嵌套，且同艺人物品对的共现余弦是随机的 **176×**（结构确实在，但 item 表已经在学它）；
  参数代价 artist **530 万**（+13%）/ album **2,190 万**（+55%）。
  ② **外推**——离线口径把冷物品记 `-1`（占比 1.34% / 1.48%），真新艺人只有 253 / 298 个物品；embedding 表里没有那个 id，对它无效。
  ③ **下手空间小**——“新歌·稀疏(≤5)” 是三个模型全为 0 的死区，但只占总目标 3.2% / 5.8%，且其中只有 ≈35% 的行“用户历史里已有同艺人”
  → 上限约占总目标 2%，总指标上看不见。
  ④ 真要动这块死区只有**音频嵌入**（`embeddings.parquet`，阶段 4）对症，但它要下 13.8 GB，且**冷物品在当前候选池外**，
  要量得单开一条口径（与历史表不可比）——**是否做、以及要不要开这条口径，等用户定**。
...
| `is_organic` 事件特征默认关（`USE_ORGANIC`） | val −1.0% / test −1.5%（主指标），只换来新歌@10 +14~17%；+128 参数（占 item embedding 0.003%）不是容量问题 → 归因到信号本身 |
| 目标侧三个臂（A/B/C）都只看“新歌@10”变好 | 三臂的主指标没有一个稳定变好，而 66.6% 的目标是回访 → 若要真做新歌轴，应直接盯 `新歌@10` 这类指标（且先想清楚它值多少） |
