...
3. 切分：官方 train.py / eval.py 都用 val_size=0（第二遍训练集）并在 test 上报告；我们在第一遍切分的
   train 上训练，val 用来选超参、test 用来报告，与 popularity / itemknn 同一口径。

两个 split：`val` 训练并评测（同时存 checkpoint），`test` 不重训、直接加载那份 checkpoint。
...
SELECT_K = 100
SPLIT = sys.argv[1] if len(sys.argv) > 1 else "val"  # 评测目标：val（训练+评测）或 test（复用 checkpoint）
...
def main() -> None:
    ...
    if SPLIT == "val":
        ...
        model.load_state_dict(torch.load(CHECKPOINT, map_location=INFER_DEVICE))
    ...
    metrics, n_targets = evaluate(list(tops[target_users]), target_chunks, n_items)
    show(f"[{SPLIT}] 官方口径（不过滤已交互物品、目标保留不可排名行）", metrics, n_targets, n_items)
