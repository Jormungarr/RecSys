...
| 字段 | 实测 |
|---|---|
| `listens.is_organic` | 推荐驱动（0）占 **48.3%**（论文报 48.74%）；按正负样本拆开是 **Listen+ 52.03% / Listen− 41.97%**（train 里 52.01%）——推荐推的歌反而更容易被“听完”；**目标侧的回访占比在两类之间差很多**（可排名目标：主动发现 80.87% / 推荐驱动 56.06%）→ 单看这一列做结论会被回访混淆（见 `docs/baselines.md`「分层轴」） |
...
**艺人 / 专辑的共现结构很强**（2026-09-22 只读核算）：同艺人物品对的用户集合余弦相似度 **0.313** vs 随机物品对 **0.0019**（**163×**；
同专辑 **0.449** / **238×**；采样 400 个有 2~50 首的艺人 / 专辑，seed 固定）。→ 这既是“item 表自己能学出艺人结构”的证据，
也是艺人 / 专辑 embedding 与 item 表**共线**的来源（见 `docs/baselines.md`「长尾/稀疏度分层 + D 可行性检查」）。
复现：`uv run python scripts/07_zone_checks.py`（只读、不训练；这篇检查是用户要求入库留档的）。
